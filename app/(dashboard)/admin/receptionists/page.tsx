    ...
    <DashboardShell allowedRoles={[ROLES.ADMIN]}>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500">Add or remove receptionist accounts</p>
        </div>
      ...
      {showForm && (
        <Card className="mb-6">
          <form onSubmit={handleAdd} className="space-y-4">
            <h3 className="font-semibold text-gray-900">New Receptionist</h3>
      ...
      {loading ? (
        ...
        <Card>
          {receptionists.length === 0 ? (
            <p className="text-center py-8 text-gray-500">No receptionists registered.</p>
          ) : (
                ...
                <tbody>
                  {receptionists.map((rec) => (
                    <tr key={rec._id} className="border-b border-gray-100 hover:bg-gray-50">
                      <td className="py-3 font-medium text-gray-900">{rec.name}</td>
                      <td className="py-3 text-gray-600">{rec.email}</td>
